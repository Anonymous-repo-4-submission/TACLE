...
class DataManager(object): # _train() in trainer.py calls this class
    ...
    # following 3 arguments are added: tasks =self.tasks, task_idx=self._cur_task, buffer_lst = self.buffer_lst, unsupervised
    def get_dataset(self, indices, source, mode, tasks , task_idx, buffer_lst, appendent=None, ret_data=False, with_raw=False, with_noise=False, keep_file= None, compute_mean = False, unsupervised = False): # incremental_train() in slca.py calls this function
        if source == 'train':
        ...
        if not unsupervised:
            ...
            for idx, indx in enumerate(cur_cls):
                ...
                # imbalanced
                # new_targets.extend(y[indices[0][:imbalance_num_samples[idx]]])
                # new_samples.extend(x[indices[0][:imbalance_num_samples[idx]]])

                # logging.info(f"class: {indx} || num_unlableled_samples: {len(indices[0][:imbalance_num_samples[idx]])}")
                # breakpoint()
