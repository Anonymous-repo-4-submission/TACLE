...
class BaseLearner(object): #Called by SLCA in slca.py
    ...
    def _compute_accuracy(self, model, loader, unsupervised =  False): # Called  _run.py by SLCA in slca.py
        model.eval()
        correct, total = 0, 0
    ...
    def _extract_vectors_unlabel_conf(self, loader): #compute_class_mean() calls it
        self._network.eval()
        vectors, targets = [], []
    ...
    def _compute_class_mean(self, data_manager, check_diff=False, oracle=False, unsupervised_dset_loader=None, strategy =None): # Called by incremental_train() in SLCA in slca.py
        ...
        for class_idx in range(self._known_classes, self._total_classes): # 0, 10 -> 10, 20 
            ...
            idx_loader = DataLoader(idx_dataset, batch_size=batch_size, shuffle=False, num_workers=4)
            # idx_loader_unsup = DataLoader(idx_dataset_unsup, batch_size=batch_size, shuffle=False, num_workers=4)
            # On changing shuffle= True ->  false No effect
